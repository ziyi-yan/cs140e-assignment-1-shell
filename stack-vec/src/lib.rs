...
impl<'a, T: Clone + 'a> StackVec<'a, T> {
    ...
    pub fn pop(&mut self) -> Option<T> {
        if self.len <= 0 {
            ...
            let i = self.len - 1;
            self.len -= 1;
            Some(self.storage[i].clone())
        }
    }
}

// Implement `Deref`, `DerefMut`, and `IntoIterator` for `StackVec`.
impl<'a, T> core::ops::Deref for StackVec<'a, T> {
    type Target = [T];
...
impl<'a, T> core::ops::DerefMut for StackVec<'a, T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        self.as_mut_slice()
    }
}

impl<'a, T: Clone + 'a> core::iter::IntoIterator for StackVec<'a, T> {
    type Item = &'a T;
...
impl<'a, T: Clone + 'a> core::iter::IntoIterator for &'a StackVec<'a, T> {
    type Item = &'a T;
    type IntoIter = core::slice::Iter<'a, T>;
    fn into_iter(self) -> Self::IntoIter {
        self.iter()
